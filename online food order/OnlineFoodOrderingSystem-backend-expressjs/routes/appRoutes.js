...
function secondtimecart(req, res, oldcart, newrecipe, userid) {
    var oldavail = false;
   
    var newtotal = oldcart.total + newrecipe.price;
    var tot;
   
    var oldrecipejsonarr = oldcart['recipe']
   
    for (var i = 0; i < oldrecipejsonarr.length; i++) {
        if (oldrecipejsonarr[i]._id == newrecipe._id) {
            oldavail = true;
        }
    }
    ...
    if (oldavail) {
        ...
        for (var i = 0; i < oldrecipejsonarr.length; i++) {
            if (oldrecipejsonarr[i]._id == newrecipe._id) {
                oldrecipejsonarr[i].qty += 1;
                oldcart.total += oldrecipejsonarr[i].price
                tot = oldcart.total
                console.log(oldcart.total);
            }
        }

        console.log(oldrecipejsonarr);

        Cart.updateOne({ _id: oldcart._id }, {
            recipe: oldrecipejsonarr,
            total: tot
        }, function (err, ct) {
            ...
            if (err) {
                ...
                res.json({ msg: "recipe added to the cart" })
            }
        })
    }
    else {
        ...
        oldrecipejsonarr.push(newrecipe)
        Cart.updateOne({ _id: oldcart._id }, {
            recipe: oldrecipejsonarr,
            total: newtotal
        }, function (err, ct) {
...
router.get('/getcartitem', verifyToken, (req, res, next) => {
    Cart.find({ whichuser: req.userId }, (err, recipes) => {
        if (err) {
...
function deletefromcart(req, res, oldcart, newrecipe, userid) {
    var temp = [];
    var total;
    var oldrecipejsonarr = oldcart['recipe']
    ...
    for (var i = 0; i < oldrecipejsonarr.length; i++) {
        if (oldrecipejsonarr[i]._id != newrecipe._id) {
            temp.push(oldrecipejsonarr[i])
        }
        else {
            oldcart.total -= newrecipe.price * newrecipe.qty;
            total = oldcart.total;
        }
    }
    if (total == 0) {
        ...
        res.json({ msg: "recipe deleted from the cart" })
